...
parser.add_argument("--format", default=None, help="Change the output format between 'standard', 'json', 'json2', and 'csv'. For the CSV output, you must supply types that you want.")
    ...
    if args.format == 'json':
        ...
        for key in data.keys():
            if type(data[key]) == array.array:
                data[key] = list(data[key])
        outMsg = {"meta": meta, "data": data}
        json2OutputArray.append( outMsg )
        # Now print out this object with stringified properly.
...
if args.format == 'json2':
    print(json.dumps(json2OutputArray))
